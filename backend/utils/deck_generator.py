...
def generate_deck(content, deck_file_path):
    try:
        ...
        logger.info(f"Starting deck generation process :{content}")
        ...
        # ========= Title Slide ==========
        title_slide = prs.slides.add_slide(prs.slide_layouts[0])
        title_slide.shapes.title.text = content["deck_title"]
        # title_slide.placeholders[1].text = content["title_slide"]["subtitle"]
        title_slide.placeholders[1].text = content["deck_title"]
        ...
        for slide_data in content["slides"]:
            ...
            if bullets:
                # ---- First bullet MUST use body.text ----
                first = bullets[0]

                if isinstance(first, dict):
                    body.text = f"{first.get('text', '')} — Impact: {first.get('impact', '')}"
                else:
                    body.text = first

                body.paragraphs[0].font.size = Pt(BODY_FONT_SIZE)
                body.paragraphs[0].font.color.rgb = TEXT_DARK
        ...
        for bullet in bullets[1:]:
            ...
            if isinstance(bullet, dict):
                p.text = f"{bullet.get('text', '')} — Impact: {bullet.get('impact', '')}"
            else:
                p.text = bullet
